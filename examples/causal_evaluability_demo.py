...
Scenario query:
    "How does EGFR inhibition affect pMEK signalling and proliferation in EGFR-driven cells?"
...
Run against the real graph:     MG_DB=/tmp/mg-demo.kuzu python examples/causal_evaluability_demo.py
"""
...
DAG = [
    HypothesisEdge(
        "H1", "high-proliferation vs low-proliferation", "EGFR expression",
        "High-proliferation lines over-express EGFR vs low.",
        "group contrast in EGFR mRNA (high − low proliferation)",
    ...
    HypothesisEdge(
        "H2", "EGFR kinase activity", "pMEK (S217/221)",
        "EGFR inhibition lowers pMEK.",
        "ATE of do(EGFR inhibited) on pMEK intensity",
        design="interventional", estimator_method="m:limma",
        strategy="experimental intervention (erlotinib vs vehicle / shEGFR vs scramble), randomized across wells",
        causal_assumptions=["exchangeability (by randomized perturbation)",
                            "positivity (both arms realized)",
                            "consistency / SUTVA  ⚠ paracrine ligand (amphiregulin) may break no-interference"],
        refutations=["vehicle + scramble control", "orthogonal inhibitor (gefitinib) replication",
                     "MEK-rescue: constitutively-active MEK (MEK-DD) abolishes the effect", "dose–response monotonicity"],
        id_evidence="Pearl, Causality (do-operator); Hernán & Robins, What If (exchangeability)",
    ),
    HypothesisEdge(
        "H3", "pMEK (S217/221)", "proliferation rate",
        "Baseline pMEK predicts proliferation (as measured by correlation).",
        "effect of pMEK on proliferation",
        design="observational", estimator_method=None,
        strategy="",  # none as stated
        causal_assumptions=["exchangeability — VIOLATED: confounded by MYC amplification, RB1/p16 status"],
        refutations=[],
        id_evidence="",
        fix="intervene on the mediator: do(pMEK) via trametinib (MEK-i) or MEK-DD (constitutive) — "
            "converts the edge to an interventional, identifiable design",
    ),
    HypothesisEdge(
        "H4", "EGFR inhibition + MEK inhibition", "growth suppression",
        "Combined EGFR + MEK inhibition suppresses growth beyond either agent alone.",
        "interaction: shift in growth dose–response under combined EGFR + MEK inhibition (ΔGI50 / synergy)",
        design="interventional", estimator_method=None,   # no dose-response tool node in KG
        strategy="factorial combination (EGFR-inhibitor dose-series ± MEK inhibitor)",
    ...
    HypothesisEdge(
        "H5", "EGFR-high proliferation", "cell-cycle (E2F) program",
        "EGFR-high proliferation rides a cell-cycle (E2F) transcriptional program.",
        "enrichment of a cell-cycle / E2F signature in EGFR-high / high-proliferation",
        design="descriptive", estimator_method="m:gsea",
        strategy="gene-set enrichment (association)",
        causal_assumptions=[],
        refutations=["independent E2F signature", "permutation null"],
...
def main() -> None:
    ...
    elif Path("/tmp/mg-demo.kuzu").exists():
        db_path = Path("/tmp/mg-demo.kuzu")
        print(f"methods broker: real graph at {db_path}")
    else:
    ...
    banner('QUERY  "How does EGFR inhibition affect pMEK signalling and proliferation in EGFR-driven cells?"')
    print("\nThe target is a MEDIATION estimand:  EGFR ─▶ pMEK ─▶ proliferation")
    print("  natural indirect effect through pMEK — identified only under sequential")
    print("  ignorability (cross-world); refutation = does MEK-DD rescue abolish the")
    print("  anti-proliferative effect?  Testable implication of 'pMEK is the sole mediator':")
    print("     EGFR ⟂ proliferation | pMEK   (a conditional-independence test of the DAG)")

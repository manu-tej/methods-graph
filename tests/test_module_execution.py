...
def _write_named_module(root: Path, name: str, main_nf: str) -> Path:
    d = root / "modules" / "nf-core" / name
    d.mkdir(parents=True)
    (d / "main.nf").write_text(main_nf)
    (d / "meta.yml").write_text(f"name: {name}\n")
    return root
...
# Singularity-only: both ternary branches are https blob URLs, so there is no bare
# docker registry path — parse_container yields docker=None, singularity=<url>.
_SINGULARITY_ONLY_MAIN = '''process TD2 {
    container "${ workflow.containerEngine == 'singularity'
        ? 'https://community-cr-prod.seqera.io/docker/registry/v2/blobs/sha256/41/4155bf3b/data'
        : 'https://community-cr-prod.seqera.io/docker/registry/v2/blobs/sha256/41/4155bf3b/data' }"
    script:
    """
    TD2.LongOrfs
...
# No container directive at all (e.g. nf-core dragen — runs on FPGA hardware).
_NO_CONTAINER_MAIN = '''process DRAGEN {
    script:
    """
    dragen --help
    """
...
def test_build_emits_spec_for_singularity_only_module(tmp_path):
    """A spec pinning ONLY a singularity image is runnable and must still be emitted."""
    root = _write_named_module(tmp_path, "td2_longorfs", _SINGULARITY_ONLY_MAIN)
    nodes, edges, rep = build_execution_records(
        [root], {"mod:td2_longorfs"}, ingested_at="2026-06-19")
    assert rep.specs == 1 and len(nodes) == 1 and len(edges) == 1
    assert not (nodes[0].properties.get("container") or "")          # no docker image
    assert nodes[0].properties["container_singularity"].startswith("https://")  # but a real one


def test_build_skips_module_with_no_container_image(tmp_path):
    """A module with NO image (neither docker nor singularity) gets no ExecutionSpec —
    it is not a runnable recipe — but the skip is recorded, never silent."""
    root = _write_named_module(tmp_path, "dragen", _NO_CONTAINER_MAIN)
    nodes, edges, rep = build_execution_records(
        [root], {"mod:dragen"}, ingested_at="2026-06-19")
    assert nodes == [] and edges == []
    assert ("mod:dragen", "no_container_image") in rep.skipped

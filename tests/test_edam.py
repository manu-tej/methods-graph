...
def test_parse_edam_stores_synonyms_for_keyword_search(tmp_path):
    """Synonyms from the pipe-separated 'Synonyms' column land in node properties so
    keyword search matches real alternative terms (e.g. 'GSEA'), not only the label."""
    tsv = tmp_path / "syn.tsv"
    tsv.write_text(
        "Class ID\tPreferred Label\tParents\tObsolete\tSynonyms\n"
        "http://edamontology.org/operation_2436\tGene-set enrichment analysis\t\t\t"
        "GSEA|Functional enrichment analysis\n"
        "http://edamontology.org/operation_2495\tGene expression analysis\t\t\t\n",
        encoding="utf-8",
    )
    nodes, _ = parse_edam(tsv, ingested_at="2026-06-18")
    by_id = {n.id: n for n in nodes}
    assert by_id["op:operation_2436"].properties["synonyms"] == [
        "Functional enrichment analysis", "GSEA"]
    # a row with no synonyms carries no (empty) synonyms key
    assert "synonyms" not in by_id["op:operation_2495"].properties

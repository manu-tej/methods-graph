...
class NodeKind(str, Enum):
    ...
    MODALITY = "Modality"
    # External agent-skill library entry (BioClaw, K-Dense, …) that wraps one or
    # more Method nodes via WRAPS edges.
    SKILL = "Skill"
...
class EdgeKind(str, Enum):
    ...
    # Curated, grounded Assay→Method link: the tool that analyzes the data matrix a
    # wet-lab assay produces (e.g. RPPA / phospho-protein readout → a limma-style
    # differential-abundance tool).  Bridges an OBI assay node (otherwise orphaned)
    # to an analysis tool so an assay readout can ground to a method.  Carries an
    # evidence token (doi:/pmid:); emitted only Assay→Method, audit re-checks typing
    # + grounding.

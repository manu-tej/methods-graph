...
from methods_graph.extract.seed import (
    seed, method_neighborhood, method_ids_matching, method_preconditions,
)
...
class KuzuMethodsGraphProvider:
    ...
    def method_preconditions(self, method_id: str) -> dict[str, Any]:
        """The evaluability contract for *method_id* in one call.

        Returns ``{assumptions: [{name, evidence, checkable, threshold?, ...}],
        diagnostics: [{name, checks, ...}]}`` — everything quration's evaluability
        gate needs to decide whether a tool's statistical preconditions can be
        checked against a dataset, with thresholds (e.g. the replicate floor) and
        the pre-run/post-run flag already machine-readable.  Raises ``KeyError`` on
        an unknown method (quration maps that to a COVERAGE_GAP verdict).
        """
        return method_preconditions(self._conn, method_id)

    def skill_wraps_method_ids(self, skill_id: str) -> list[str]:
        """Method ids a Skill WRAPS, for skill-level guardrail evaluation."""
        return [r[0] for r in self._conn.execute(
            "MATCH (s:Entity {id:$id, kind:'Skill'})-[e:Rel {kind:'WRAPS'}]->"
            "(m:Entity {kind:'Method'}) RETURN m.id ORDER BY m.id",
            {"id": skill_id})]

    def resolve_skill_ids(self, keywords: list[str]) -> list[str]:
        """Skill ids whose name or description match the keywords (quration skill lane)."""
        kws = [k.lower() for k in keywords if k]
        if not kws:
            return []
        import json as _json
        rows = self._conn.execute(
            "MATCH (s:Entity {kind:'Skill'}) RETURN s.id, lower(s.name), s.properties")
        out = []
        while rows.has_next():
            sid, name, props_raw = rows.get_next()
            try:
                props = _json.loads(props_raw) if isinstance(props_raw, str) else (props_raw or {})
            except Exception:
                props = {}
    ...
    def skill_preconditions(self, skill_id: str) -> dict[str, Any]:
        """Aggregate evaluability contract for a Skill (over its WRAPS methods), in the
        same shape as method_preconditions so quration treats skills like methods.
        Uses max-merge by (id, source) so the strictest threshold wins."""
        from methods_graph.guardrail import merge_skill_preconditions
        pres = []
        for mid in self.skill_wraps_method_ids(skill_id):
            try:
                pres.append(self.method_preconditions(mid))
            except KeyError:
                # Mirror the guardrail's skill path: a stale / evaluability-less WRAPS
                # target is skipped, so one bad edge can't crash the quration-facing
                # aggregation (or `skills --coverage`, which loops this over every skill).
                continue
        return merge_skill_preconditions(pres, skill_id)
...
def build_analysis_method(method_dict: dict[str, Any]):
    ...
    try:
        ...
        raise RuntimeError(
            "quration is not installed; install quration in this environment to "
            "validate AnalysisMethod objects"
        ) from e

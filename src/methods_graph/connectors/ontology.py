...
# Synonym / alternative-term predicates.  OBI files acronyms (e.g. "RPPA assay")
# under IAO_0000118 ("alternative term"); STATO/OBI also use the oboInOwl synonym
# predicates.  Collecting these into the node's properties lets keyword search match
# real terms ("RPPA"), not only the single rdfs:label Preferred term.
_SYNONYM_PREDICATES = (
...
def parse_ontology_owl(
    ...
    *,
    ...
    # Collect synonyms / alternative terms (acronyms like "RPPA") per class.
    synonyms: dict[str, set[str]] = {}
    for pred in _SYNONYM_PREDICATES:
        for s, _p, o in g.triples((None, URIRef(pred), None)):
            if isinstance(s, URIRef):
                text = str(o).strip()
                if text:
                    synonyms.setdefault(str(s), set()).add(text)
    ...
    for iri, kind in classified.items():
        ...
        props = {"uri": iri, "ontology": source}
        syns = sorted(synonyms.get(iri, ()))
        if syns:
            props["synonyms"] = syns
...
def parse_stato(
    ...
    *,
    ...
    Roots → NodeKind.STATISTICAL_METHOD:
      OBI_0200000 ("data transformation") — captures ~193 STATO descendants
        including t-test, ANOVA, regression, and differential-expression methods.
      STATO_0000142 ("correlation coefficient") — captures Pearson's / Spearman's /
        Kendall's / poly- & tetrachoric / Matthews correlation coefficients.  STATO
        files these under "measure of correlation" (an effect-size measure), NOT
        under the "data transformation" root, so without this second root they would
        be absent and correlation/co-expression operations (op:operation_3463) would
        have no groundable downstream statistic.
    ...
    return parse_ontology_owl(
        ...
        roots={
            "OBI_0200000": NodeKind.STATISTICAL_METHOD,
            "STATO_0000142": NodeKind.STATISTICAL_METHOD,
        },
    )
